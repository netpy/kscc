...
// 带宽控制的结构体，每个任务组会有一个该结构体的成员
// 注意：实际的带宽控制将会下沉到任务组的各个cfs_rq中去落实，因此cfs_rq也包含了与带宽控制相关的字段
// 详见cfs_rq
struct cfs_bandwidth {
#ifdef CONFIG_CFS_BANDWIDTH
	raw_spinlock_t lock;
	ktime_t period;                               // 一个周期的时长
	u64 quota, runtime;                           // 一个周期内的时间限额，本周期内剩下的可用时间
	s64 hierarchical_quota;

	short idle, period_active;
	// 高精度定时器：
	// 前者每个period内定时更新runtime；
	// 后者是回收时间的定时器
	struct hrtimer period_timer, slack_timer;
	// 挂起队列，所有被throttled的cfs_rq挂到该链表上，
	// 在定时器的回调函数中遍历该链表执行unthrottle操作。
...
struct cfs_rq {
	...
	// 是否开启带宽控制
	int runtime_enabled;
	// 当前cfs_rq从task_group中分配到的时间配额的剩余量，
	// 如果该时间小于0,则需要重新从task_group中申请时间.
	// 每当更新任务的时间时,该字段也会更新,在update_curr函数中实现
	s64 runtime_remaining;

	// 记录cfs_rq被throllte时的时间点,用于统计被throllte的时间
	u64 throttled_clock, throttled_clock_task;
	u64 throttled_clock_task_time;

	// 前者表示当前cfs_rq是否被throllte
	// 后者标记当前cfs_rq被throllte的次数,
	// 注意,由于调度组层次的存在,
	// 如果上层task_group被throllte时,该数字也会增加.
	int throttled, throttle_count;
	// 本队列中被throllte的子cfs_rq被throllte时,
	// 会被挂入这个队列中
