...
void set_bt_load_weight(struct task_struct *p)
{
	...
	// 通过进程的调度实体获得struct load_weight
	struct load_weight *load = &p->bt.load;

	// 根据优先级查表，设置权重，使用的表的cfs相同
	load->weight = scale_load(sched_prio_to_weight[prio]);
	// inv_weight存储2^32/ weight，所以查的是不同的表
...
/* cpu runqueue to which this bt_rq is attached */
// 通过bt_rq得到所在CPU的就绪队列
...
// 一个bt调度实体，是否是一个进程，有可能是一个组调度
#define bt_entity_is_task(se)	(!se->bt_my_q)

// 根据调度实体获得进程task_struct
static inline struct task_struct *bt_task_of(struct sched_entity *bt)
{
#ifdef CONFIG_SCHED_DEBUG
// WARN_ON_ONCE()会将调试信息输出到dmesg，类似于BUG_ON()打印的内容。与BUG_ON()的区别在于，WARN_ON_ONCE()仅会在第一次触发时打印调试信息，并且继续执行。
...
// 	将bt rq中若干task组织成若干task group，即子bt_rq，属于这group的task所使用到的处理器时间就会以个group总共所分的的时间为上限。 
// 2.基于cgroup的bt group scheduling 架构
// 	2.1 可以创造出有阶层性的task组织，根据不同task的功能群组化。 
// 	2.2 在配置给该群组对应的处理器资源，让属于该群组下的task可以通过rq机制使用属于该群组下的资源。 
// 3.加入、退出、遍历leaf_bt_rq_list
// 	3.1 加入：list_add_leaf_bt_rq把一个group bt rq加入。
// 	3.2 退出：list_del_leaf_bt_rq把一个group bt rq退出。
//	3.3 遍历：for_each_leaf_bt_rq遍历rq上得所有leaf bt_rq
static inline void list_add_leaf_bt_rq(struct bt_rq *bt_rq)
{
	/* 修复修复由于缺失负载衰减引起的不公平而引入的:
	 * 如果需要 CFS 支持组调度管理，那就得把所有 CFS 加入到一个链表当中，
	 * leaf_cfs_rq_list 成员就是负责把本CPU下的就绪队列中各个 CFS 子队列关联起来。
	 * 并且在 cfs_rq 里面有成员 on_list，
	 * 其表示当前的 CFS 队列 leaf_cfs_rq_list 成员是否加入到就绪队列 rq 的 leaf_cfs_rq_list 管理的链表当中
	 */
	if (!bt_rq->on_list) {
		/*
...
/* 进程唤醒的时候会调用check_preempt_wakeup检查是否可以抢占当前进程，
 * 显然如果在task_fork_fair我们已经确定了子进程将抢占父进程，那么这里就直接返回；
 * 如果当前进程是idle进程（该函数不一定只检查父子进程之间的抢占与否，只是在现在的场景是这样的），那么它应该无条件被抢占，更新当前进程的执行时间，
 * 在确定是否要抢占前必须保证比较的两个se处于同一group之下(find_matching_se，即找到它们第一次分开的祖先为止，只有处理同一个层次它们才有可比较性)，
 * 然后再判断这两个同级的se（可能是之前两个se的祖先），旧的se是否可以被新的se抢占(wakeup_preempt_entity)，
 * 如果允许的话，更新set_next_buddy，这样在pick_next_entity时它就可以优先被获得；
 * 最后就是调用resched_task设置当前进程的TIF_NEED_RESCHED，以及更新set_last_buddy(这个的作用同样见pick_next_entity)。
 * 所以这个函数就是用来找到他们第一次分开的祖先的。
 */
static void
...
// 通过所在离线负载的调度队列找到所在CPU的就绪队列
static inline struct rq *rq_of_bt_rq(struct bt_rq *bt_rq)
{
	return container_of(bt_rq, struct rq, bt);
}

// 由于没有开启组调度，所以所有调度实体都是进程，所以直接定义为1即可
...
/* runqueue "owned" by this group */
// 通过当前所在调度实体获得对应的组调度运行队列
static inline struct bt_rq *group_bt_rq(struct sched_entity *grp)
{
	return NULL;
}

// 同上，但是由于此处并没有开启组调度，也就不需要使用该函数，直接定义为空即可
static inline void list_add_leaf_bt_rq(struct bt_rq *bt_rq)
{
}
...
// 这里仅仅做了声明，用于带宽控制，
/* 在带宽控制器中，有两个定时器，分别是period_timer和slack_timer
 * 前者用来周期性地更新带宽时间并对cfs_rq解挂（结束挂起）
 * 后者用来酌情回收已经分配给下属cfs_rq的时间 
 * 具体实现可见如下的几个函数：
 * */
 ...
 * 带宽控制的总体思路如下：
 * 系统为任务组设置带宽额度，
 * 任务组中的各个cfs_rq首先需要向任务组（task_group）申请时间，
 * 如果申请成功，则cfs_rq中的任务可以被CFS调度运行，
 * 如果申请失败（例如当前周期内任务组的时间配额已经用完），
 * 则调度器会将cfs_rq挂起（throttle）；
 * 系统通过定时器周期性地为任务组重新分配时间额度，
 * 并将挂起的cfs_rq解挂（unthrottle）。 
 */
